...
namespace DetailPortraits.Dialog {
    ...
    public class Dialog_PresetList : Window {
        ...
        private List<DetailPortraitsPreset> presets = new List<DetailPortraitsPreset>();
        ...
        public override void DoWindowContents(Rect inRect) {
            ...
            float num2 = 0f;
            int num3 = 0;
            foreach (DetailPortraitsPreset current in this.presets) {
                ...
                GUI.color = Dialog_PresetList.DefaultPresetTextColor;
                ...
                if (Widgets.ButtonText(rect4, "DetailPortraits.LoadPreset".Translate(), true, false, true)) {
                    Load(current);
                }
                ...
                TooltipHandler.TipRegion(rect5, "DetailPortraits.DeleteThisPreset".Translate());
                GUI.EndGroup();
                num2 += vector.y + 3f;
                num3++;
            }
        ...
        public void Save(DetailPortraitsPreset preset) {
            this.presets.Add(preset);
            SaveSettings();
        }

        public void Remove(DetailPortraitsPreset preset) {
            this.presets.Remove(preset);
            SaveSettings();
        }

        public void LoadSettings() {
            this.presets = DetailPortraitsPref.presets;
            if (this.presets == null) {
                Log.Message("DetailPortraitsPreset is null");
                this.presets = new List<DetailPortraitsPreset>();
            }
        }

        public void SaveSettings() {
            DetailPortraitsPref.presets = this.presets;
            DetailPortraitsPref.SavePref();
        }
        ...
        public void DoTypeInField(Rect rect) {
            ...
            if (Widgets.ButtonText(rect3, "DetailPortraits.SavePresetButton".Translate(), true, false, true) || flag) {
                if (this.typingName.NullOrEmpty()) {
                    Messages.Message("NeedAName".Translate(), MessageTypeDefOf.RejectInput);
                } else {
                    this.Save(new DetailPortraitsPreset(this.typingName, this.portraitData));
                }
            }
            Text.Anchor = TextAnchor.UpperLeft;
            GUI.EndGroup();
